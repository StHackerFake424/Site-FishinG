...
def print_startup():
    ...
    print("\033[00m")
    sys.stdout.write("\t  .:[ Fakei1 last version (%s-%s) ]:.\n\033[00m" %(__version__,  __codename__,))
...
def shell():
    ...
    while True:
        try:
            # for Re-complete
            complete(array)
            an = raw_input("Enter help > ") or "help"
            prompt = an.split()
            if not prompt:
              continue
            elif prompt[0] == ";" or prompt[0] == "clear":
                print("\033[H\033[J")
            elif prompt[0] == "q" or prompt[0] == "exit":
                printt(2,"good bye!")
                break;
                ...
                print("\tset action_url   : %s " %(action_url))
                print("\tset user_agent   : %s " %(user_agent))
                print("\tset html_file    : %s " %(html_file))
                print("\tset external_js  : %s " %(external_js))
                sys.stdout.write("\t")
                print("-" * 20)
